...
import { getSchema } from "../../utils/seoHandler";
import FAQContent from "./components/FAQContent";

export default function FAQ({ data, firstSection = true }) {
	const { pageName, ourFAQData, faqOthers, faqSectionSchema } = data || {};
	const { faqSectionTitle } = faqOthers || {};

	const title = "Frequently Asked Questions";

	return (
		<section
			aria-label={`${pageName} page faq section`}
			className="relative bg-primary-dark"
		>
			{getSchema(faqSectionSchema)}
			<div
				className={`container ${
					firstSection === true
